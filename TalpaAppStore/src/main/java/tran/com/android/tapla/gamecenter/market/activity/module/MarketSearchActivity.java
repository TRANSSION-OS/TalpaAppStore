...
//
	...
	private void upDownLoadDataFro9Apps(AppListObject m_obj) {
		...
		for (int i = 0; i < m_obj.getData().getApps().size(); i++) {
			...
			tmp_data.setPublishId(list.getPublishId());
			tmp_data.setDownloadAddress(list.getDownloadAddress());
			tmp_data.setIcon(list.getIcon());
			tmp_data.setApkName(list.getTitle());
			tmp_data.setPackageName(list.getPackageName());
			tmp_data.setVersionCode(list.getVersionCode());
		...
		mListView.setOnItemClickListener(new OnItemClickListener() {
			...
			public void onItemClick(AdapterView<?> parent, View view,
				...
				if (mListView.getAdapter().getItem(position) != null) {
					...
					intent.putExtra("downloaddata", ((DownloadData) mListView.getAdapter().getItem(position)));
