...
import tran.com.android.tapla.gamecenter.market.activity.picbrowser.PictureViewActivity;
import tran.com.android.tapla.gamecenter.market.activity.setting.DownloadManagerActivity;
...
import tran.com.android.tapla.gamecenter.market.service.AppInstallService;
...
//	public ImageView main_update;
	...
	public final int IMAGE_HEIGHT=238;
	...
	public void setScrollAnim(int scrollAnimY) {
		...
		float alpha = 1 - scrollAnimY / (heightPixels / 16);
		mappavatar.setAlpha(alpha);
		mappavatar.setScaleX(alpha);
		mappavatar.setScaleY(alpha);
		float alpha1 = 1 - scrollAnimY / (heightPixels / 8);
		mAppname.setAlpha(alpha1);
		mRatingBar.setAlpha(alpha1);
		mDownloadCount.setAlpha(alpha1);
		mAppSize.setAlpha(alpha1);
		mActionBar.setTitleString(mAppname.getText());
		Log.v("heightPixels","heightPixels:"+scrollAnimY);
		if(scrollAnimY>SHOW_ACTIONBAR_ANIMATION){
			if(!isStartAnimato){
				ObjectAnimator.ofFloat(mActionBar.getTitleView(), "alpha", 0, 1)
						.setDuration(500)
						.start();
				isStartAnimato=true;
			}
		}else if(scrollAnimY<HIDDEN_ACTIONBAR_ANIMATION){
			if(isStartAnimato){
				ObjectAnimator.ofFloat(mActionBar.getTitleView(), "alpha", 1, 0)
						.setDuration(500)
						.start();
				isStartAnimato=false;
			}
		}
//		mActionBar.setTitleAnim(scrollAnimY / (heightPixels / 8) - 1);
	}
	...
	private void initimageLoad() {
		...
		optionsImage = new DisplayImageOptions.Builder()
				...
				.showImageOnFail(R.drawable.page_thumbnail).cacheInMemory(false)
	...
	private void disViewFrom9Apps() {
		...
		mDownloadCount.setText(info.getDownloadTotal()
				+ getResources().getString(R.string.download_count_str));
		mAppSize.setText(info.getFileSize() + "KB");
		mAppVersion.setText(info.getVersionName());
...
//		mAppDeveloper.setText(info.get.getDeveloper());
		mAppCategory.setText(info.getCategory());
...
//		mActionBar.setTitle(R.string.app_detail_page);
		...
		mActionBar.setIcon(getResources().getDrawable(R.drawable.page_detail_back_bar));
